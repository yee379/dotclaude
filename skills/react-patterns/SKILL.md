...
### Queries with generated types

```tsx
// Use graphql-codegen — always type your operations
...
// component
function UserList() {
  const { data, loading, error, fetchMore } = useQuery<GetUsersQuery, GetUsersQueryVariables>(
    GET_USERS,
    { variables: { first: 20 } }
  );
  ...
  return (
    <>
      {data?.users.edges.map(({ node }) => <UserCard key={node.id} user={node} />)}
...
const [deleteUser] = useMutation<DeleteUserMutation, DeleteUserMutationVariables>(DELETE_USER, {
  optimisticResponse: ({ id }) => ({
    deleteUser: { __typename: "DeleteUserPayload", userId: id, errors: [] },
  }),
  update(cache, { data }) {
    if (!data?.deleteUser.errors.length) {
      cache.evict({ id: cache.identify({ __typename: "User", id: data.deleteUser.userId }) });
      cache.gc();
    }
  },
