...
When used in a project that has a `research/` workspace, load the tool index first:

```
Read: research/tools/index.json
```

Then load only the schemas you need:

```
Read: research/tools/schemas/<tool_name>.json
```

Never load all schemas at once — a typical session needs 2–3.

### Technique 2 — Programmatic Tool Calling

When a task requires multiple workspace queries before deciding whether to spawn
subagents, write a Python code block to batch them off-context:

```python
import sys
sys.path.insert(0, "research/tools")
from research_tools import search_concepts, search_reports, list_topics, blast_radius

existing = search_concepts("vector semantic search embedding")
reports  = search_reports("vector search")
blocked  = list_topics(status="blocked")
priority = blast_radius(["vector", "semantic", "embedding"])

print("existing_concepts:", existing["total"])
print("blocked:", [t["topic"] for t in blocked["topics"]])
...
| Single Bash command | Use Bash tool directly |
| 2+ workspace queries | Write Python |
| Pre-flight check before spawning subagents | Write Python |
| Session-start orientation on existing project | Write Python |

### Technique 3 — Tool Use Examples

Every schema in `research/tools/schemas/` includes an `examples` array. Read it before
calling a tool — examples show exact parameter values and return shapes.

### Available tools (when research/ workspace exists)

| Tool | Use case |
|------|----------|
| `search_concepts` | Check for existing concept entries before cataloguing |
| `search_reports` | Check for existing reports before writing |
| `get_concept` | Read a concept without knowing its path |
| `get_report` | Read a report without knowing its path |
| `list_topics` | Session orientation — blocked items, todo queue |
| `blast_radius` | Compute priority before adding a TOPICS.md entry |
